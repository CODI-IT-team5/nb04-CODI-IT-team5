...
export async function getCartItemsForOrder(userId: string, cartItemIds?: string[]) {
  ...
  const where: { cartId: string; id?: { in: string[] } } = { cartId: cart.id };
...
// 주문 완료 후 Cart 아이템 삭제 (특정 아이템들 또는 전체)
export async function clearCartItems(userId: string, cartItemIds?: string[]) {
  const cart = await findCartByUserId(userId);
  if (!cart) return 0;
